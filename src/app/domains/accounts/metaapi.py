...
class MetaApiService:
    ...
    def _get_deals_window(
        ...
        url = (
            f"{settings.METAAPI_CLIENT_BASE_URL}/users/current/accounts/"
            f"{account_id}/history-deals/time/{from_value}/{to_value}"
        )
    ...
    def get_account_info(self, account_id: str) -> dict[str, Any]:
        url = (
            f"{settings.METAAPI_CLIENT_BASE_URL}/users/current/accounts/"
            f"{account_id}/account-information"
        )
        with self._client() as client:
