...
from app.models.post import Post, PostMediaType
from app.models.stream import StreamPrivacy
from app.models.stream_member import MemberStatus
from app.models.user import User
from app.repositories import post_media_repo, post_repo, post_upvote_repo, stream_member_repo, stream_repo
...
def _build_response(
    ...
    *,
    current_user: User,
    # Callers that already know the parent's deleted state (e.g. list_replies,
    # which fetched the parent for its own 404 guard) pass it in to avoid a
    # redundant DB round-trip per item in the page.
    ...
    upvote_count = post_upvote_repo.count(db, post.id)
    has_upvoted = post_upvote_repo.get(db, user_id=current_user.id, post_id=post.id) is not None

    # Determine if this reply's parent has been soft-deleted.
    # Use the pre-fetched value when provided; otherwise do a single DB lookup.
...
def _check_stream_view_access(
    ...
    """
    Raise 403 if the user cannot view/interact with posts in the stream.
    Public streams: any authenticated user.
    Private / paid streams: must be active member OR the owner.
    """
...
def create_reply(
    ...
    *,
    ...
    # ── disallow reply-to-reply (keep threading flat like Twitter) ───────────
    ...
    # ── check viewer access on the stream ────────────────────────────────────
...
def list_replies(
    ...
    *,
    ...
    # Check that the requesting user can see this stream
    _check_stream_view_access(db, parent.stream_id, current_user)
    ...
    # parent.is_deleted is already known from the 404 guard above — pass it in
...
def delete_post(
    ...
    *,
    ...
    # Author OR stream owner may delete
...
def upvote_post(
    ...
    *,
    ...
    # Idempotent — silently succeed if already upvoted
    existing = post_upvote_repo.get(db, user_id=current_user.id, post_id=post_id)
    if existing is None:
        _check_stream_view_access(db, post.stream_id, current_user)
        post_upvote_repo.create(db, user_id=current_user.id, post_id=post_id)
        db.commit()
        db.refresh(post)
...
def unupvote_post(
    ...
    *,
    ...
    if existing is not None:
        post_upvote_repo.delete(db, existing)
        db.commit()
        db.refresh(post)
